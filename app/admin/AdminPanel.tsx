...
type SettingsPayload = { settings?: SiteSettings; telegram?: { configured: boolean; chatId: string } };

async function fetchAdminData<T>(url: string): Promise<T> {
  const response = await fetch(url, { cache: "no-store" });
  const payload = await response.json();
  if (!response.ok) throw new Error(payload.error || "Ma’lumotlarni yuklab bo‘lmadi.");
  return payload;
}
...
export default function AdminPanel({ email }: { email: string }) {
  ...
  const loadCatalog = useCallback(async (selectedDate: string) => {
  ...
  async function refresh() {
    ...
    setMessage("");
    try {
      await Promise.all([loadOrders(), loadCatalog(date), loadSettings()]);
    } catch (error) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      fetchAdminData<{ orders?: AdminOrder[] }>("/api/admin/orders"),
      fetchAdminData<{ dishes?: AdminDish[] }>(`/api/admin/catalog?date=${date}`),
      fetchAdminData<SettingsPayload>("/api/admin/settings"),
    ])
      .then(([orderData, catalogData, settingsData]) => {
        if (!active) return;
        setOrders(orderData.orders ?? []);
        setDishes(catalogData.dishes ?? []);
        setSettings(settingsData.settings ?? defaultSiteSettings);
        setTelegramConfigured(Boolean(settingsData.telegram?.configured));
        setTelegramChatId(settingsData.telegram?.chatId ?? "");
      })
      .catch((error: unknown) => {
        if (active) setMessage(error instanceof Error ? error.message : "Ma’lumotlarni yuklab bo‘lmadi");
      })
  ...
  async function updateOrder(id: number, status: "accepted" | "cancelled") {
    ...
    try {
      ...
      await Promise.all([loadOrders(), loadCatalog(date)]);
    } catch (error) {
  ...
  async function saveDish(dish: AdminDish) {
    if (imageUpload.current !== null) return;
    setSaving(dish.id);
    try {
  ...
  async function createDish(event: FormEvent<HTMLFormElement>) {
    ...
    if (imageUpload.current !== null || imageErrors.new) return;
    setSaving("new");
    try {
  ...
  async function uploadImage(file: File, target: "new" | number) {
    if (imageUpload.current !== null || saving !== null) return;
    const targetKey = String(target);
    setImageErrors((current) => ({ ...current, [targetKey]: "" }));
    if (!file.size || file.size > 4 * 1024 * 1024) {
      const error = "Rasm bo‘sh bo‘lmasin va hajmi 4 MB dan oshmasin.";
      setImageErrors((current) => ({ ...current, [targetKey]: error }));
      setMessage(error);
      return;
    }
    imageUpload.current = targetKey;
    setUploading(targetKey);
    setMessage("");
    try {
      const form = new FormData();
      form.append("file", file);
      if (target !== "new") form.append("dishId", String(target));
      const response = await fetch("/api/admin/uploads", { method: "POST", body: form });
      const payload = await response.json().catch(() => ({})) as { error?: string; url?: string; saved?: boolean };
      ...
      if (target === "new") setNewDish((current) => ({ ...current, imageUrl: payload.url || "" }));
        ...
        if (!payload.saved) throw new Error("Rasm taomga saqlanmadi. Sahifani yangilab, qayta yuklang.");
        updateDish(target, "imageUrl", payload.url);
      }
      setMessage(target === "new" ? "Rasm tayyor. Endi “Taom qo‘shish” tugmasini bosing." : "Rasm taomga saqlandi. Menyudagi taom rasmi asosiy sahifada ham yangilanadi.");
    } catch (error) {
      const detail = error instanceof Error ? error.message : "Rasm yuklanmadi";
      setImageErrors((current) => ({ ...current, [targetKey]: detail }));
      setMessage(detail);
    } finally {
